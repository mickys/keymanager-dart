...
void main() async {
  ...
  final myKeyManager = KeyManager(mnemonic);
  ...
  test('Generates all 6 keys', () {
    ...
    expect(myKeyManager.keys["storage_root_privateKey"]?.length, 66);
    expect(myKeyManager.keys["comms_privateKey"]?.length, 66);

    expect(myKeyManager.keys["wallet_publicKey"]?.length, 130);
    expect(myKeyManager.keys["storage_root_publicKey"]?.length, 130);
  ...
  String signedMessage2 = "0x2c4672e19bd21e85d27b6c61dd37b009f2a191f3b7e943870c4d7a97d49164ae0035b510e1e86d9b6dbf53e2cd5817cce0d88833647ce33cf70279cdb0c7354d1c";
  test('signPersonalMessage returns correct string', () {

    String signed = KeyManagerUtils.signPersonalMessage(msg, key);
    expect(signed, signedMessage2);
  });

  test('getPublicKeyFromSignature returns correct address', () {
    String address = KeyManagerUtils.getPublicKeyFromSignature(msg, signedMessage2);
  ...
  // print("storage_root_privateKey:   ${myKeyManager.keys["storage_root_privateKey"]}");
  // print("storage_root_publicKey:    ${myKeyManager.keys["storage_root_publicKey"]}");

  // print("storage_vdb_privateKey:    ${myKeyManager.keys["storage_vdb_privateKey"]}");
  // print("storage_vdb_publicKey:     ${myKeyManager.keys["storage_vdb_publicKey"]}");

  // print("storage_vdc_privateKey:    ${myKeyManager.keys["storage_vdc_privateKey"]}");
  // print("storage_vdc_publicKey:     ${myKeyManager.keys["storage_vdc_publicKey"]}");
